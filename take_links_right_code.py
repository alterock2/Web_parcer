...
def mnain():
    ...
    for category in categories:
        ...
        for product in products_list:
            ...
            colors = soup.find_all('div', class_='color-item')
            product_data['Дополнительные цвета'] = ''
            for i in colors:
                product_data['Дополнительные цвета'] += i.text.strip() + '\n'
            #description1
            safe = soup.find('div', class_='description-text')
            product_data['Описание'] = safe
    ...
    df.to_excel('products_data_right.xlsx', index=False)
